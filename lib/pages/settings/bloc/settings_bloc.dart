  ...
  Stream<ChanState> mapEventToState(ChanEvent event) async* {
    try {
      if (event is SettingsEventSetTheme) {
        ...
        int appThemeIndex =
            Preferences.getInt(Preferences.KEY_SETTINGS_THEME) ?? 0;
        ...
        for (BoardItem board in boards) {
          List<ThreadItem> threads =
              await _localDataSource!.getThreadsByBoardId(board.boardId);
          if (threads.isEmpty) {
            continue;
          }
          ...
          MoorBoardOverview boardsOverview = new MoorBoardOverview(
            ...
            threads
                .where(
                    (thread) => thread.onlineStatus == OnlineState.ONLINE.index)
                .length,
            threads
                .where((thread) =>
                    thread.onlineStatus == OnlineState.ARCHIVED.index)
                .length,
            threads
                .where((thread) =>
                    thread.onlineStatus == OnlineState.NOT_FOUND.index)
                .length,
            threads
                .where((thread) =>
                    thread.onlineStatus == OnlineState.UNKNOWN.index)
                .length,
          );
