...
class DiskCache {
  ...
  int _maxSizeBytes = 1024 * 1024 * 256; // default: 256 MB
      ...
      if (_metadata.containsKey(uid)) {
        ...
        if (DateTime.fromMillisecondsSinceEpoch(
              _metadata[uid]['createdTime'] +
                  (cacheRule != null
                      ? cacheRule.maxAge.inMilliseconds
  ...
  Future<bool> save(String uid, Uint8List data) async {
    if (_metadata == null) await _initMetaData();
    Directory dir = Directory(join(
        (cacheRule.storeDirectory == StoreDirectoryType.temporary
    ...
    try {
      ...
      Map<String, dynamic> metadata = {
        ...
        'crc32': cacheRule.checksum ? crc32(data) : null,
        'size': data.lengthInBytes,
        'maxAge': cacheRule.maxAge.inMilliseconds,
      };
  ...
  Future<void> _checkCacheSize() async {
    while (currentEntries > maxEntries || _currentSizeBytes > maxSizeBytes) {
      ...
      print("key: $key, currentEntries: $currentEntries, maxEntries: $maxEntries, _currentSizeBytes: $_currentSizeBytes, maxSizeBytes: $maxSizeBytes");
