...
class ChanTransitionToImage extends StatefulWidget {
  const ChanTransitionToImage({
    Key key,
    @required this.image,
    this.width,
    this.height,
    ...
    this.loadingWidget = const Center(child: Constants.progressIndicator),
  ...
  @override
  _ChanTransitionToImageState createState() => _ChanTransitionToImageState();
}
...
class _ChanTransitionToImageState extends State<ChanTransitionToImage> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    if (widget.transitionType == TransitionType.fade) {
      ...
      _slideTween = widget.tween ?? Tween(begin: const Offset(0.0, -1.0), end: Offset.zero);
    }
  ...
  @override
  void didUpdateWidget(ChanTransitionToImage oldWidget) {
  ...
  void _getImage({bool reload: false}) {
    ...
    if (_imageProvider is ChanNetworkImage && widget.loadingWidgetBuilder != null) {
      var callback = (_imageProvider as ChanNetworkImage).loadingProgress;
      (_imageProvider as ChanNetworkImage).loadingProgress = (double progress, List<int> data) {
        if (mounted) {
          setState(() {
            _progress = progress;
  ...
  Widget buildPlaceholderWidget(BuildContext context) {
  ...
  Widget buildLoadingWidget(BuildContext context) {
    return widget.loadingWidgetBuilder != null ? widget.loadingWidgetBuilder(context, _progress, _imageData) : widget.loadingWidget;
  }
